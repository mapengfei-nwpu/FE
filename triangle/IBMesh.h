...
public:
	IBMesh(std::array<dolfin::Point, 2> points, std::vector<size_t> dims, CellType::Type cell_type = CellType::Type::triangle)
	{
		...
		// generate mesh
		auto _mesh = RectangleMesh::create(points, {dims[0], dims[1]}, CellType::Type::triangle);
	...
	bool check()
	{
		for (CellIterator cell(*mesh_ptr); !cell.end(); ++cell)
		{
			...
			std::cout << "global index: "
					  ...
					  << cell->index()
					  << " point hash: "
					  << map(hash(point)*2)[1]-cell->index()
					  << ". mpi rank: "
					  << map(hash(point)*2)[0]
					  << ". number of adjacents: "
					  << get_adjacents(point).size()
					  << std::endl;
			if (cell->global_index() / 2 != hash(cell->midpoint()))
			{
				return false;
			}
		}
