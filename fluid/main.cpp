...
class NoslipDomain : public SubDomain
{
  ...
  {
    return (on_boundary && (x[0] < DOLFIN_EPS || x[1] < DOLFIN_EPS ||
                            x[0] > 1.0 - DOLFIN_EPS));
  }
};
...
int main()
{
  ...
  RectangleMesh _mesh(point0, point1, 32, 32);
  ...
  auto v_in = std::make_shared<InflowPressure>();
